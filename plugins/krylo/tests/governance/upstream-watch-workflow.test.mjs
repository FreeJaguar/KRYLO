// Workflow-security static tests for upstream-watch.yml (Weekly Upstream
// Watch, docs/adr/0036-weekly-upstream-watch.md). Mirrors
...
const WORKFLOW_PATH = path.join(REPO_ROOT, '.github', 'workflows', 'upstream-watch.yml');
...
test('upstream-watch workflow: exists', () => {
  assert.ok(fs.existsSync(WORKFLOW_PATH));
});

test('upstream-watch workflow: weekly schedule plus manual dispatch, per design Section 15.2', () => {
  ...
  assert.notEqual(fields[4], '*', 'a weekly watch must pin a day-of-week, not run daily');
  assert.equal(fields[2], '*', 'day-of-month must stay wildcard for a weekly cadence');
  assert.match(text, /^\s+workflow_dispatch:/m, 'manual dispatch must be available');
});

test('upstream-watch workflow: never triggered by push or pull_request, so it can never block a PR or a release', () => {
  const text = workflowText();
...
test('upstream-watch workflow: contents: read at the top level, and nothing broader anywhere', () => {
  const text = workflowText();
...
test('upstream-watch workflow: checkout does not persist credentials', () => {
  assert.match(workflowText(), /persist-credentials:\s*false/, 'checkout must disable credential persistence');
});

test('upstream-watch workflow: every action is pinned to a full 40-hex commit SHA', () => {
  const text = workflowText();
...
test('upstream-watch workflow: no PAT or repository-write credential is referenced', () => {
...
test('upstream-watch workflow: never installs or executes anything belonging to a watched candidate', () => {
  const text = workflowExecutableText();
  for (const forbidden of ['npm install', 'npm ci', 'npx ', 'pip install', 'yarn add', 'pnpm add', 'git clone', 'curl ', 'wget ']) {
    assert.ok(!text.includes(forbidden), `design Section 15.5 forbids ${forbidden.trim()} in this workflow`);
  }
});

test('upstream-watch workflow: never writes to the repository, opens a PR, or files an issue', () => {
  const text = workflowExecutableText();
...
test('upstream-watch workflow: invokes the real read-only checker entrypoint', () => {
  assert.match(workflowText(), /node plugins\/krylo\/scripts\/maintenance\/check-upstream-watch\.mjs/, 'the workflow must run the shipped checker');
});
