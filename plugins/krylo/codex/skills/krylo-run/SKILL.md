...
3. **Session identifier -- known, disclosed platform limitation, read carefully:** current Codex documentation does not expose the real session identifier to you directly anywhere (confirmed open upstream gap, `openai/codex#8923`), and current Codex hook-output validation rejects every mechanism KRYLO's own PreToolUse Hook could otherwise use to bridge a Hook-visible session id back into a command you run (confirmed by direct inspection of the installed Codex build). **Do not invent, guess, generate, or hardcode a session id.** If the runtime genuinely exposes your real session identifier to you through some other means you can directly observe (check first; do not assume it does not), use that exact value for every `--session` argument below. If it does not -- the current default -- **stop here**: do not run `init-run.mjs`. Explain to the user that KRYLO's Codex host cannot currently establish reliable session binding on this Codex build, so an autonomous `$krylo-run` cannot start safely, and this requires either a platform capability update or explicit user guidance on how to proceed. This is a genuine, disclosed capability gap (`docs/codex-capability-matrix.md`), not something to route around.
4. Start the run (use a short normalized goal, never the raw prompt; substitute your confirmed real session id from step 3 -- never a placeholder):

   ```text
   node "${PLUGIN_ROOT}/scripts/runtime/init-run.mjs" --goal "<short goal>" --session "<your confirmed real session id>" --lane <LANE> --risk <risk>
   ```

   If a run for this project is already active, resume it instead: `node "${PLUGIN_ROOT}/scripts/runtime/read-state.mjs" --session "<your confirmed real session id>"`. Always pass the exact same real session id on every `update-state.mjs`/`read-state.mjs` call below.
5. Compile observable acceptance criteria before modifying application code, and record each one:

   ```text
   node "${PLUGIN_ROOT}/scripts/runtime/update-state.mjs" --session "<your confirmed real session id>" --add-criterion "<criterion>"
   ```
...
7. Use safe, conventional, reversible defaults instead of asking routine questions. Grant an exceptional token before an allowed-category question:
   ...
   ```text
   node "${PLUGIN_ROOT}/scripts/runtime/update-state.mjs" --session "<your confirmed real session id>" --grant-question <category>
   ```
...
8. Track phases with `--phase EXECUTING|VERIFYING|REVIEWING|CORRECTING`, and record every verification result as evidence with real output summaries:
   ...
   ```text
   node "${PLUGIN_ROOT}/scripts/runtime/update-state.mjs" --session "<your confirmed real session id>" --add-evidence '{"type":"test","label":"unit tests","sourceTool":"npm test","result":"pass","summary":"<real counts>"}'
   ```
...
9. Run applicable deterministic verification, then obtain independent review. Mark criteria proven only with passing evidence:
   ...
   ```text
   node "${PLUGIN_ROOT}/scripts/runtime/update-state.mjs" --session "<your confirmed real session id>" --set-criterion AC-1=proven --evidence EV-1
   ```
