...
2. Confirm the host-initialized run is active and read its current state:
   ...
   If this reports no active run, host-side bootstrap did not complete (a disclosed, confirmed platform-dependent condition -- see `docs/codex-capability-matrix.md`). **Stop here.** Explain to the user that KRYLO's Codex `UserPromptSubmit` Hook did not initialize a run for this invocation, and this requires investigating the Hook's own trust/registration state rather than attempting to work around it (never call `init-run.mjs` yourself to compensate).
