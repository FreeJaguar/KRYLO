...
10. Production, destructive, financial, release, identity, secret, and external-write actions are gated at the risk gate. A Bash `git push` (plain or `--force`) triggers Claude Code's own native permission prompt only in a session whose permission mode is `auto`, `manual`, or `default` (the three live-verified as honoring it), and the human decides directly through that host UI, not through anything you can set yourself (`--resolve-approval <id>=approved` always fails when called from here). Every other gated action -- any other class (production deployment, database writes, destructive operations, package publication, release creation, merge, IAM/secrets, payment, external messages/writes), any PowerShell command, any MCP tool call, or a `git push` in any other permission mode (including `bypassPermissions`, `plan`, `acceptEdits`, `dontAsk`, or one KRYLO cannot identify) -- is denied outright instead (no native prompt is used for those); if a human should review and unblock one, record it with `--request-approval <actionClass> --summary "<summary>"` and end with `--terminal RISK_APPROVAL_REQUIRED`. You may use the same `--request-approval`/`RISK_APPROVAL_REQUIRED` path for any class you judge sensitive enough to pause the whole run for deliberate human review before even attempting it.
