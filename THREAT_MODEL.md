...
- A new host adapter's PreToolUse deny path must never emit a decision shape the target host has confirmed unsupported (Codex: `permissionDecision:"ask"`, legacy `decision:"approve"`, `continue:false`, `stopReason`, `suppressOutput`) -- `risk-gate-codex.mjs`/`permission-request-codex.mjs` have no code path capable of producing any of them. Re-verified against the current stable Codex release (not the older baseline every other Codex document here was checked against): an unsupported `permissionDecision` now confirmed to fail OPEN (the tool call proceeds) rather than merely "fail the hook" -- this makes the "no code path capable of producing it" guarantee more clearly load-bearing, not less.
- **Codex project-scoped hook enforcement (`docs/adr/0032-codex-project-scoped-hook-enforcement.md`) introduces a project-local trust surface distinct from the plugin-bundled hooks.** `<repo>/.codex/hooks.json` and its companion launcher (`<repo>/.codex/krylo/codex-project-hook-launcher.mjs`) may be git-tracked and shared with teammates; both remain subject to Codex's own per-hook trust review (`/hooks`) exactly like the plugin-bundled file -- KRYLO setup never marks them trusted or bypasses that flow. The launcher itself carries no policy (it only locates and re-executes the real, already-installed enforcement scripts by a portable, non-machine-specific algorithm); if that real runtime cannot be found at all, the `PreToolUse` launcher fails closed (denies) rather than silently no-op, since a missing runtime means it cannot determine whether a run is even active.
- **A newer, older, or otherwise unreviewed Codex runtime could behave differently from the exact version(s) KRYLO's hook contracts were verified against, silently invalidating those guarantees (`docs/adr/0034-codex-runtime-compatibility-gate.md`).** A concrete instance was found and disclosed while building this control: `SessionEnd` (registered in `plugins/krylo/hooks/codex-hooks.json` per ADR-0033, verified only against `rust-v0.152.1` SOURCE) is confirmed, by direct byte-inspection, absent from the actually-installed `codex-cli 0.120.0` binary's own embedded hook-event strings. The Runtime Compatibility Gate does not and cannot fix that specific gap (it runs inside a hook process, so if a plugin-bundled hooks file failed to load at all, no KRYLO hook including this gate would ever fire) -- its actual guarantee is narrower and different: before a full autonomous `$krylo-run` bootstraps, the installed runtime's `codex --version` output is checked against a reviewed, exact-match-only compatibility contract (`plugins/krylo/policies/codex-runtime-compatibility.json`); anything not explicitly reviewed there is `SAFE_BLOCKED` rather than silently trusted. No network dependency, no caching, no model-influenceable input.
