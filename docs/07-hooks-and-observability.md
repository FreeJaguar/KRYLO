...
The risk gate's `permissionDecision` is `allow` for a policy-passed action, `deny` for an outright policy denial (data-root protection, Hook-entrypoint protection, a protected secret path, an oversized command), and, for a `require-approval` classification (production, destructive, publish, release, push, merge, IAM/secret, payment, and other declared write classes): `ask` only when the tool is Bash, the action class is `git-push` or `git-force`, and `permission_mode` is in the `ASK_ELIGIBLE_PERMISSION_MODES` allowlist (`auto`, `manual`, `default` -- the only modes live-verified to honor a Hook's `ask` decision; docs/adr/0025-native-permission-approval.md) -- `deny` in every other case (a different tool, a different action class, or a permission mode outside that allowlist, including `bypassPermissions`, `plan`, `acceptEdits`, `dontAsk`, an absent field, or any future/renamed mode). This split is not a policy gap -- risk *classification* is identical across tools -- it is because official Claude Code documentation only confirms the underlying `ask` guarantee narrowly (Bash, "auto mode", scoped to what CHANGELOG entries actually state), and KRYLO does not extend a trusted mechanism past what is documented (see `docs/adr/0025-native-permission-approval.md` and `docs/adr/0026-powershell-risk-parity.md`). Every case outside that narrow scope keeps the deterministic `deny` fail-safe, which is strictly more conservative than an unconfirmed `ask`. Both genuine fail-safe paths (an unreadable/malformed Hook payload, or an exception while classifying the tool call) also return `deny`: those are not `require-approval` decisions with a legitimate human-review outcome, they are KRYLO's own inability to classify the action at all.
