...
- "safe capability probe success" / "safe capability probe contradiction" -> **not mapped to a test.** No secondary local capability probe was built this checkpoint (the two candidates considered, a binary string-scan and `codex features list`, were evaluated and explicitly rejected -- see ADR-0034's own "second, real finding" section and the design doc's "explicitly out of scope" list). `codex --version` itself is both the probe AND the sole compatibility signal in this design, so its own success/failure is already exhaustively covered by Task 2/3 -- there is no SEPARATE secondary probe whose success/contradiction against the primary signal needs its own test. Recorded here explicitly, not silently omitted, per an independent Verifier's own finding that this table should say so rather than leave the gap unstated.
