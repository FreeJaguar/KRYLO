...
- `tests/unit/risk-policy.test.mjs`: the new contract file and `runtime-compat.mjs` entrypoint protections (already covered by existing `touchesPluginInstallation()`/`HOOK_ENTRYPOINT_FILENAMES` mechanisms -- new regression cases naming the new file explicitly).
