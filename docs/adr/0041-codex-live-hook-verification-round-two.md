...
- `host/codex/context.mjs`: `deriveCodexDataRootFromPluginRoot()` (new, exported for direct test), `resolveCodexDataRoot()` gains one fallback tier.
- `scripts/lib/host-dispatch.mjs`: `detectHost()` recognizes `CODEX_THREAD_ID`.
...
- New tests: `tests/unit/host-dispatch.test.mjs` (5 cases); `tests/unit/codex-context.test.mjs` gains 6 cases covering the derivation function and its fallback ordering; `tests/hooks/user-prompt-submit-codex.test.mjs`'s two existing literal-path regressions were corrected to assert the data root is ABSENT from both messages, plus one new end-to-end case that bootstraps a run under a real, correctly-shaped temp directory structure and proves a later `read-state.mjs` call finds it from the plugin root alone.
