...
Path rules map to design Section 15.4's analysis dimensions — `package-lifecycle`, `install-scripts`, `hooks`, `plugin-manifests`, `mcp-inventory`, `license`, `binaries`, `credentials-or-network-config`, and `agent-instructions` (Section 15.4's "Skill inventory changes"). Any hit escalates to `SECURITY_REVIEW_REQUIRED`. `DRIFT_LOW_RISK` requires that EVERY changed path carry an inert prose extension — an extension allowlist with no directory arm, for the reason recorded under "Review findings" below. Anything else is `REVIEW_REQUIRED`.
...
Every transport failure — timeout, network error, rate limiting, HTTP error, malformed JSON — resolves to `SOURCE_UNAVAILABLE` *while drift is still unestablished*, and to `REVIEW_REQUIRED` once drift is already known but the change list could not be read. `SOURCE_UNAVAILABLE` outranks `NO_DRIFT` when computing the run's headline verdict, so a run that could not see upstream never presents itself as healthy. The security-relevant invariant is the one asserted by test: **no transport failure and no malformed-but-`ok:true` body can ever surface as `NO_DRIFT`.** Drift whose change list could not be read stays `REVIEW_REQUIRED` rather than being downgraded. An empty changed-file list is also `REVIEW_REQUIRED`, not low risk: the refs differ by definition at that point, so "no files" means the comparison did not explain the difference (GitHub truncates very large diffs), and treating an unexplained difference as safe is precisely the silent downgrade this subsystem exists to prevent.
...
The 31 unit tests are fully hermetic (injected client), covering every classification, every transport-failure reason, offline mode, the ref-spelling fallback and both of its refusals, agent-executable Markdown, executable code under a documentation prefix, an identical-refs compare, a throwing client, and control characters crafted to forge a report line.

## Review findings, and what they changed

An independent Security Reviewer, dispatched against the first implementation, reproduced four real defects with executed proof-of-concepts. All are fixed here; each has a regression test, because without one they would land back in the same untested gaps that let them through.

- **The trust boundary was inverted where it mattered most (High).** The baseline ref came from the trusted catalog while the repository being *observed* came from the watch config, with nothing asserting they agree — so a one-line edit to the "configuration-only, therefore low-stakes" file (a typo, a fork URL, a hostile PR) pointed the watch at a different repository whose matching tag then reported an affirmative `NO_DRIFT`, silencing that integration permanently. This is the precise inversion design Section 15.3 exists to prevent: the separation is meant to keep low-trust data from acquiring authority, and the low-trust file had *sole* authority over the observation target. Now the trusted catalog decides which repository is observed, and a disagreement between the two files is surfaced as `SOURCE_UNAVAILABLE` rather than silently resolved in either direction.
- **`.md` is not documentation in this ecosystem (High).** `SKILL.md`, agent definitions and slash-command files are instructions an agent executes, and a frontmatter change alone can grant `allowed-tools: Bash`. The flagship watch entry, `mattpocock/skills`, is a repository made almost entirely of such files — so its single most security-relevant change classified as "documentation-only, you can skip this". The old rule also treated *anything* under `docs/`/`examples/` as documentation regardless of extension, which made `docs/build.sh` and `examples/server.js` low risk. Low risk is now an inert-extension allowlist, and a new `agent-instructions` rule escalates skill/agent/command Markdown. The live run confirms this was not hypothetical: `mattpocock-skills` now reports `agent-instructions` against real upstream data.
- **The one credible source of recurring noise (Medium).** `3.8.49` versus `v3.8.49` is the same commit, but strict string inequality treated it as drift, the compare then returned an identical (empty) result, and the empty-list rule reported `REVIEW_REQUIRED` every week with a reason — "change list unavailable or truncated" — that was simply untrue. The disambiguating data was already in hand and being discarded. An `identical` compare is now `NO_DRIFT`, while a genuinely empty file list on a non-identical compare stays `REVIEW_REQUIRED`.
- **Upstream filenames could forge report lines (Medium).** Git permits `LF` in a path name and the API returns it verbatim, so a crafted filename could reproduce this report's own line format and inject a fabricated `NO DRIFT` entry into the rendered report and the GitHub job summary — which renders as Markdown. Control characters are now stripped before truncation, and the workflow writes the summary inside a fenced block (which also stops the report's column alignment from collapsing).

Smaller fixes from the same round: the `v`-prefix fallback now requires at least two dots and is case-sensitive, so a recorded `3` is never retried as `v3` (often a *moving* alias tag); the escalation rules gained `Gemfile`/`composer.json`/`go.sum`/`poetry.lock`/`*.gemspec`, `.yarnrc`/`.pypirc`/`Dockerfile.*`/`.github/workflows/*`/`.claude/settings.json`, and `.deb`/`.msi`/`.pyc`/versioned sonames; an implausibly long path (>512 characters) escalates rather than being matched at all, which also removes the measured quadratic backtracking; and a client that throws now degrades that one entry to `SOURCE_UNAVAILABLE` instead of losing every other entry's result with it.

### Deliberately not changed, disclosed instead

Each watch entry's `watch` array (e.g. `["skill-list", "hooks", ...]`) and its `cadence` field are **declarative intent only** — the checker applies the same global rule set to every entry, and cadence lives in the cron. `reviewedRefSource: "tools.json"` is likewise not read: the trusted catalog path is hard-coded, which is the safer property (the field cannot be repointed at another file) but means the declaration is documentation rather than configuration. Per-entry rule filtering was judged unnecessary complexity for three entries and is recorded here rather than implied to work.
