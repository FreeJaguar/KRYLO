...
- **Isolated by construction.** A temporary `CODEX_HOME` (with only `auth.json` copied in, deleted again afterwards), a throwaway project directory, an isolated `KRYLO_DATA_ROOT`, and — for the standalone-runtime path — an isolated `KRYLO_TEST_HOME`. **No KRYLO-relevant state in the real `~/.codex` was modified**: its `config.toml`, `hooks.json`, and installed-plugin state are unchanged by this checkpoint (verified by mtime afterwards; `config.toml` and the plugin staging directory carry the earlier, separately-authorized install from the same day). Stated precisely rather than as a blanket claim: Codex *did* write its own bookkeeping files there (`.codex-global-state.json`, `logs_2.sqlite*`) as an unavoidable side effect of the read-only informational commands run before the sandbox existed (`codex --version`, `codex login status`, `codex features list`). One further deviation is disclosed under "Process note" below.
