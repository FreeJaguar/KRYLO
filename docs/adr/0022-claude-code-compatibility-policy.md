...
No CHANGELOG entry through the current released version extends the underlying auto-mode-`ask`-flooring guarantee to PowerShell or to any MCP tool's permission dialog by that exact version-pinned bug-fix trail. ADR-0025 originally treated this absence as reason to narrow native `ask` to Bash `git-push`/`git-force` only; ADR-0027 supersedes that narrowing, reasoning instead from official documentation's general statement that PreToolUse hooks apply identically to every tool (see ADR-0027 for the full argument and its own disclosed evidence limits).
