...
- Human-approval authority belongs to the host's own native permission UI, not to any KRYLO-local state a prompt-injected model could forge or manipulate. For the Bash tool, a required-approval action routes to Claude Code's native `permissionDecision: "ask"` prompt (`docs/adr/0025-native-permission-approval.md`); for every other tool where that same host guarantee is not yet officially confirmed (PowerShell, MCP), the action is denied deterministically instead (`docs/adr/0026-powershell-risk-parity.md`) -- never merely gated behind a KRYLO-local approval record that could authorize it on its own. A KRYLO-local risk-approval record (`docs/adr/0019-scoped-single-use-approvals.md`'s scoped/bound/single-use design) may still exist for audit and run-completion bookkeeping, but cannot independently authorize execution.
