...
- **Ecosystem Maintenance (ADR-0031) is read-only, official-source-only, and non-authoritative.** The drift checker never edits a repository file, commits, pushes, or opens/modifies an issue or PR; every `fetch`-based upstream call is restricted to a fixed GitHub API domain allowlist enforced on every redirect hop (also checking scheme and port, not only hostname), bounded (10s timeout covering the full request including the body read, 2MB response cap, max 3 redirects, at most one retry for a network-level failure only); a network failure becomes an explicit `unavailable` result, never a fabricated "up to date." The scheduled workflow runs `contents: read` only, requires no secret or PAT, and never triggers on `push`/`pull_request`. A detected drift is evidence for a separate, human-approved maintenance task, never an automatic remediation.
  - **Disclosed limitation: the dependency category's `npm audit --json` subprocess makes its own, separate network call to the configured npm registry, which is NOT covered by the `fetch`-client's domain allowlist above.** Not exploitable today (no `.npmrc` exists in this repository to redirect that traffic, and `package.json` declares zero dependencies), but a distinct network path from the one this checker's own allowlist governs. See ADR-0031's second review-round section.
- **On the Codex Host, `require-approval` denies deterministically instead of prompting.** Current official Codex `PreToolUse` documentation confirms `permissionDecision: "ask"` is parsed but unsupported and fails the hook if returned -- unlike Claude, Codex has no verified in-hook mechanism to produce (and wait on) the same real human decision. A fresh re-verification against the current stable Codex release confirms this is a fail-OPEN condition if ever misused (the tool call proceeds), not merely "the hook fails" -- KRYLO's Codex risk gate has no code path capable of emitting `"ask"`, by construction. KRYLO's Codex risk gate therefore denies every `require-approval` classification -- shell, `apply_patch`, or MCP alike -- regardless of tool or `permission_mode`, rather than fake approval parity. A separately generated, KRYLO-owned Codex project rule may let an operator get Codex's own native prompt for a small set of shell-command shapes in an ordinary (non-KRYLO) session, but KRYLO's own gate does not trust or depend on that rule, since the rules/`PreToolUse`/`approval_policy` execution order is not documented upstream. See `docs/codex-capability-matrix.md` for the full capability-by-capability comparison.
- **Codex project-scoped hook enforcement (`docs/adr/0032-codex-project-scoped-hook-enforcement.md`) never bypasses Codex's own hook trust.** `install-codex.mjs --target hooks` writes `<repo>/.codex/hooks.json` and a thin, policy-free launcher (`<repo>/.codex/krylo/codex-project-hook-launcher.mjs`); it does not, and cannot, mark either as trusted -- the user must review and trust them through Codex's own `/hooks` flow before they ever fire, identical to the plugin-bundled hooks. The launcher redirects to the real enforcement scripts a `--target skill` install bundles at `$HOME/.agents/skills/krylo-run/`; if that runtime is missing entirely (never installed, or removed after hooks were set up), the `PreToolUse` launcher fails closed (denies) rather than silently no-op, since it cannot otherwise determine whether a run is active.
- **A Codex runtime is not trusted merely for being installed or for `codex --version` succeeding (`docs/adr/0034-codex-runtime-compatibility-gate.md`).** Before a full autonomous `$krylo-run` bootstraps on Codex, KRYLO evaluates the installed runtime against a small, git-tracked, human-reviewed compatibility contract (`plugins/krylo/policies/codex-runtime-compatibility.json`, exact-version match only -- no semver-range trust) using a single bounded, local, non-destructive probe (`codex --version`; no network access, no model invocation, no repository source ever leaves the process). Every unreviewed, blocked, or unverifiable runtime -- older or newer than anything reviewed, a missing/unresolvable executable, a probe timeout, malformed version output, or a malformed compatibility registry -- fails safe: the run is still created (a real, inspectable audit record, never silent nothing) but saved already `SAFE_BLOCKED`, with no active-run pointer ever written for it, so it never receives autonomous execution. The model has no channel to influence this verdict: the gate never reads prompt content, and no compatibility claim the model might assert in its own output is ever parsed or trusted.
