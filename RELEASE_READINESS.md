...
- ADR-0025: native, host-controlled permission approval replaces the KRYLO-APPROVE chat-phrase mechanism (ADR-0024, superseded), narrowly for the Bash tool's `git-push`/`git-force` classes and only when `permission_mode` is in the live-verified allowlist (`auto`, `manual`, `default` -- not merely "outside `bypassPermissions`"); raised the CLI floor to 2.1.223 (2.1.211 confirms the core fix; 2.1.223 additionally closes a related Bash permission-check bypass). Sensitive-path protection (`.env`, keys, credential stores) now also covers Read/Glob/Grep, not just Bash/Write.
